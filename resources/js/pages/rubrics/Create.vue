...
<template>
    <Head title="Upload Defense Rubric" />
        ...
        <Card class="mx-auto w-full max-w-xl">
            <CardHeader>
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <CardTitle>Upload Defense Rubric</CardTitle>
                        <CardDescription>Upload the official defense rubric PDF for {{ subject.title }}</CardDescription>
                    </div>
                </div>
            </CardHeader>
            <CardContent>
                <form @submit.prevent="submit" class="flex flex-col gap-5" enctype="multipart/form-data">
                    <div class="flex flex-col gap-2">
                        <Label for="defense_period_id">Defense Round <span class="text-destructive">*</span></Label>
                        <Select
                            v-model="periodChoice"
                            :disabled="canReplaceSelectedLockedRubric"
                        >
                            <SelectTrigger id="defense_period_id" class="h-12 w-full rounded-xl border-[#24327a]/15 bg-background px-4 text-base shadow-sm">
                                <SelectValue placeholder="Choose Midterm, Final, or custom round" />
                            </SelectTrigger>
                            <SelectContent class="rounded-xl">
                                <SelectItem v-for="period in defensePeriods" :key="period.id" :value="String(period.id)">
                                    {{ period.name }}{{ period.rubric ? period.rubric.status === 'locked' ? ' (locked)' : ' (replace existing rubric)' : '' }}
                                </SelectItem>
                                <SelectItem v-if="!canReplaceSelectedLockedRubric" value="custom">Custom evaluation round</SelectItem>
                            </SelectContent>
                        </Select>
                        ...
                        <p class="text-xs text-muted-foreground">Midterm, Final, and custom rounds can use different official rubrics.</p>
                    ...
                    <div class="flex flex-col gap-2">
                        <Label for="pdf">Defense Rubric (PDF) <span class="text-destructive">*</span></Label>
                        <label
                            for="pdf"
                        ...
                        >
                            ...
                            <div v-else class="flex flex-col items-center gap-2">
                                ...
                                <p class="text-xs text-muted-foreground">Accepted file type: PDF only. Maximum size: 10 MB.</p>
                            </div>
                            <input
                                id="pdf"
                                type="file"
                    ...
                    <div class="rounded-lg bg-blue-50 px-4 py-3 text-xs text-blue-700 dark:bg-blue-950 dark:text-blue-300">
                        <strong>What happens next?</strong> AI converts the uploaded Defense Rubric file into a Dynamic Rubric draft. You can then review, edit, verify, and lock it before judges use it for scoring.
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        ...
                        >
                            ...
                            {{ form.processing ? 'Uploading...' : canReplaceSelectedLockedRubric ? 'Upload New Defense Rubric' : 'Upload Defense Rubric' }}
                        </Button>
                    </div>
                </form>
            </CardContent>
