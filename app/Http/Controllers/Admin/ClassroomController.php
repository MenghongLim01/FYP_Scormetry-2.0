...
class ClassroomController extends Controller
{
    public function index(): Response
    {
        $subjects = Subject::query()
            ...
            ->withCount(['memberships', 'teams', 'papers', 'students', 'reviewers', 'pendingMembers'])
            ->latest()
            ->get(['id', 'title', 'teacher_id', 'join_code', 'reviewer_code', 'created_at']);
    ...
    public function resetReviewerCode(Subject $subject): RedirectResponse
    {
        do {
            $code = Str::upper(Str::random(6));
        } while (Subject::where('reviewer_code', $code)->exists());

        $subject->update(['reviewer_code' => $code]);

        return back()->with('success', 'Reviewer code has been reset.');
    }
}
