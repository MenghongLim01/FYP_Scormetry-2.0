...
#[Fillable(['title', 'description', 'teacher_id', 'passing_score', 'join_code', 'reviewer_code', 'require_approval', 'archived_at'])]
class Subject extends Model
{
    ...
    /**
     * The subject-level rubric (NOT tied to a specific defense period). Used only
     * as a fallback when an attempt has no period rubric. Scoped to defense_period_id
     * = null so it never accidentally returns a period's rubric (e.g. the midterm
     * rubric leaking into a final / re-defense view).
     *
     * @return HasOne<Rubric, $this>
     */
    public function rubric(): HasOne
    {
        return $this->hasOne(Rubric::class)->whereNull('defense_period_id');
    }
