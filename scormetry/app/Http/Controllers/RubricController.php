...
class RubricController extends Controller
{
    ...
    {
        $wasLocked = $rubric->isLocked();
        $scoringStarted = $this->scoringStarted($rubric);

        if ($wasLocked && $scoringStarted && ! $request->boolean('confirm_scoring_started_change')) {
            return back()->withErrors([
                'rubric' => 'Scoring has already started. Confirm that this correction should update the locked rubric.',
            ]);
        }
        ...
        $rubric->update([
            ...
            'status' => $wasLocked ? 'locked' : 'pending_verification',
        ]);
        ...
        return to_route('rubrics.show', $rubric)
            ->with('success', $wasLocked
                ? 'Locked rubric corrected and audit history saved.'
                : 'Rubric structure updated successfully.');
    }
