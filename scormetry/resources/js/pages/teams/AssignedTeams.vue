...
import { requestReviewer as requestAttemptReviewer } from '@/actions/App/Http/Controllers/DefenseAttemptController';
...
type AvailableTeam = {
    ...
    members: Array<{ id: number; name: string; email: string }>;
    paper_status: 'available_after_approval' | 'not_uploaded';
};
...
// ── View mode ─────────────────────────────────────────────────────────────────
type ViewMode = 'assigned' | 'owner';
const viewMode = ref<ViewMode>('assigned');
const hasOwnedTeams = computed(() => props.ownedTeams.length > 0);
const showAvailableSchedule = computed(() => props.availableTeams.length > 0);
...
function formatFullDate(str: string | null): string {
    if (!str) return '—';
    return parseLocalDate(str).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
}
...
const PALETTE = [
    ...
    { block: 'bg-violet-500 border-violet-600', text: 'text-white', dot: 'bg-violet-500',  label: 'bg-violet-100 text-violet-800 dark:bg-violet-950 dark:text-violet-300' },
...
const teamColorIndex = computed(() => {
    ...
    const all = [...props.teams, ...props.ownedTeams];
    all.forEach((t, i) => map.set(t.id, i % PALETTE.length));
    return map;
});
...
function blockStyle(team: { defense_time: string | null; defense_duration: number | null }): Record<string, string> | null {
    if (!team.defense_time) return null;
    const [h, m]   = team.defense_time.split(':').map(Number);
    const topMin   = (h - GRID_START) * 60 + m;
    const duration = team.defense_duration ?? 60;
    return {
        top:    `${(topMin / 60) * HOUR_HEIGHT}px`,
        height: `${Math.max((duration / 60) * HOUR_HEIGHT, 48)}px`,
    };
}
...
function requestAssignment(team: AvailableTeam) {
    router.post(requestAttemptReviewer.url(team.id), {}, { preserveScroll: true });
}
</script>

<template>
    <Head title="Defense Schedule & Review Requests" />
    ...
    <div class="flex flex-col">
            ...
            <div class="relative flex flex-wrap items-center justify-between gap-3">
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <h1 class="text-2xl font-bold tracking-tight">Defense Schedule</h1>
                    </div>
                    <p class="mt-1.5 text-sm text-white/70">Request open teams, then review approved team rooms.</p>
                </div>
                <div class="flex items-center gap-2">
                    <div class="flex items-center gap-1.5 rounded-full bg-white/15 px-3.5 py-1.5 text-sm font-semibold ring-1 ring-white/20">
                        ...
                        {{ activeTeams.length }} assigned
                    </div>
                    <div class="flex items-center gap-1.5 rounded-full bg-white/15 px-3.5 py-1.5 text-sm font-semibold ring-1 ring-white/20">
                        <Calendar class="h-3.5 w-3.5" />
                        {{ availableTeams.length }} open
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-if="activeTeams.length === 0 && availableTeams.length === 0" class="flex flex-col items-center justify-center rounded-2xl border border-dashed bg-card py-20 text-center shadow-md">
            ...
            <p class="text-base font-semibold text-muted-foreground">No defense schedule available</p>
            <p class="mt-1 text-sm text-muted-foreground/70">Scheduled defenses will appear here when the FYP instructor opens them for review requests.</p>
        </div>

        <!-- Available schedule for reviewer self-assignment -->
        <div v-if="showAvailableSchedule" class="overflow-hidden rounded-2xl border bg-card shadow-md">
            <div class="h-1 w-full bg-gradient-to-r from-[#24327a] to-violet-600" />
            <div class="flex flex-col gap-2 border-b bg-muted/20 px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <div class="flex items-center gap-2">
                        <CalendarDays class="h-4 w-4 text-[#24327a]" />
                        <h2 class="text-sm font-bold text-[#24327a]">Available Defense Schedule</h2>
                    </div>
                    <p class="mt-1 text-xs text-muted-foreground">
                        Scheduled teams open for reviewer requests. Student documents become available after instructor approval.
                    </p>
                </div>
                <Badge variant="outline" class="w-fit border-[#24327a]/20 bg-[#24327a]/5 text-[#24327a]">
            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-[980px] text-sm">
                    <thead>
                        <tr class="border-b bg-[#24327a] text-left text-xs font-semibold uppercase tracking-wider text-white">
                    ...
                    <tbody class="divide-y">
                        <tr v-for="team in availableTeams" :key="'available-' + team.id" class="transition-colors hover:bg-muted/50">
                            <td class="px-4 py-3">
                                <div>
                                    <p class="font-semibold text-foreground">{{ team.name }}</p>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex max-w-xs flex-wrap gap-1">
                                    <span
                                        ...
                                        :key="'available-member-' + team.id + '-' + member.id"
                                        class="rounded-full border bg-background px-2 py-0.5 text-xs font-medium"
                                        :title="member.email"
                                    >
                                        {{ member.name }}
                                    </span>
                                    <span v-if="team.members.length === 0" class="text-xs text-muted-foreground">No student members</span>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap gap-1.5">
                                    <Badge variant="outline">{{ team.period.name }}</Badge>
                                    <Badge variant="secondary">{{ team.attempt_label }}</Badge>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <div v-if="team.defense_date" class="flex flex-col gap-0.5">
                                    <span class="font-medium">{{ formatFullDate(team.defense_date) }}</span>
                                    <span class="text-xs text-muted-foreground">
                                        {{ formatTime(team.defense_time) }}
                                        <template v-if="team.defense_time && team.defense_duration">
                                            - {{ addMinutes(team.defense_time, team.defense_duration) }}
                                        </template>
                                    </span>
                                </div>
                                <Badge v-else variant="outline" class="border-amber-200 bg-amber-50 text-amber-700">Waiting schedule</Badge>
                            </td>
                            <td class="px-4 py-3 text-sm">{{ team.defense_room ?? '—' }}</td>
                            <td class="px-4 py-3">
                                <Badge
                                    variant="outline"
                                    :class="team.paper_status === 'available_after_approval'
                                        ? 'border-emerald-200 bg-emerald-50 text-emerald-700'
                                        : 'border-slate-200 bg-slate-50 text-slate-700'"
                                >
                                    {{ team.paper_status === 'available_after_approval' ? 'Ready after approval' : 'Not uploaded yet' }}
                                </Badge>
                            </td>
                            <td class="px-4 py-3 text-right">
                                <Badge v-if="team.assignment_status === 'pending'" variant="outline" class="border-amber-200 bg-amber-50 text-amber-700">
                                    Request pending
                                </Badge>
                                <Badge v-else-if="team.assignment_status === 'rejected'" variant="outline" class="border-red-200 bg-red-50 text-red-700">
                                    Rejected
                                </Badge>
                                <Button v-else size="sm" class="gap-1.5 bg-[#24327a] text-white hover:bg-[#1b255c]" @click="requestAssignment(team)">
            ...
            <div v-if="displayMode === 'calendar'" class="flex">
                ...
                <div class="flex flex-1 overflow-x-auto">
                    <div
                        ...
                        class="flex min-w-0 flex-1 flex-col border-r last:border-r-0"
                        ...
                        >
                            ...
                            <template v-for="team in teamsOnDay(day).filter(t => !!t.defense_time)" :key="'blk-' + team.id">
                                <button
                                    ...
                                    class="flex cursor-pointer flex-col overflow-hidden rounded-md border px-2 py-1.5 text-left shadow-sm transition-all hover:brightness-110 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-1"
                                    :class="[teamColor(team.id).block, teamColor(team.id).text]"
                                    @click="openBlock(team)"
                                >
                                    <div class="flex items-center justify-between gap-1">
                                        <p class="truncate text-sm font-bold leading-tight">{{ team.name }}</p>
                                        <span
                                            class="h-2.5 w-2.5 shrink-0 rounded-full border border-white/40"
                                            :class="STATUS_CFG[reviewStatus(team)].dot"
                                        />
                                    </div>
                                    <p class="truncate text-xs font-medium opacity-80">{{ team.subject.title }}</p>
                                    <p v-if="team.defense_time" class="mt-auto text-xs opacity-70">
                                        {{ formatTime(team.defense_time) }}
                        ...
                        <div class="grid grid-cols-3 divide-x">
                            ...
                            <div class="flex flex-col items-center py-3 text-center">
                                ...
                                <template v-if="selected.defense_time">
                                    <p class="mt-1 text-sm font-bold tabular-nums">{{ formatTime(selected.defense_time) }}</p>
                                    <template v-if="selected.defense_duration">
                                        <p class="text-[10px] text-muted-foreground leading-none">↓</p>
                                        <p class="text-sm font-bold tabular-nums">{{ addMinutes(selected.defense_time, selected.defense_duration) }}</p>
                ...
                <div class="flex flex-1 flex-col gap-4 overflow-y-auto px-5 py-4">
                        ...
                        <div class="grid grid-cols-3 divide-x">
                            ...
                            <div class="flex flex-col items-center py-3 text-center">
                                ...
                                <template v-if="selectedOwned.defense_time">
                                    <p class="mt-1 text-sm font-bold tabular-nums">{{ formatTime(selectedOwned.defense_time) }}</p>
                                    <template v-if="selectedOwned.defense_duration">
                                        <p class="text-[10px] leading-none text-muted-foreground">↓</p>
                                        <p class="text-sm font-bold tabular-nums">{{ addMinutes(selectedOwned.defense_time, selectedOwned.defense_duration) }}</p>
                    ...
                    <div v-if="selectedOwned.paper" class="flex items-center justify-between rounded-lg border bg-muted/30 px-4 py-3">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-medium">Paper Score</span>
                        </div>
                        <div class="flex items-center gap-2">
