...
it('allows a student to join a subject using the classroom code', function () {
    $subject = Subject::factory()->create(['join_code' => 'ABCDEF', 'require_approval' => false]);
...
it('allows a reviewer to leave a subject', function () {
    ...
    $subject->reviewers()->attach($reviewer, ['role' => 'advisor']);
...
it('does not allow a reviewer to join as the FYP Instructor', function () {
    $subject = Subject::factory()->create(['reviewer_code' => 'FYPOWN']);
    $reviewer = User::factory()->teacher()->create();

    $this->actingAs($reviewer)
        ->post('/subjects/join-as-reviewer', [
            'reviewer_code' => 'FYPOWN',
