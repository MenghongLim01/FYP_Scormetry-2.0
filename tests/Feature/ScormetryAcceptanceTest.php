...
it('[5][6] lets a teacher create a subject and assign teachers as reviewers', function () {
    ...
    // Subject-level invites only offer Advisor or Custom role; fyp_instructor is
    // reserved for the owner and guest_panel is retired. Per-session scoring roles
    // (technical / academic examiner) are assigned later from the defense session.
    $this->actingAs($owner)
        ->post("/subjects/{$subject->id}/reviewers", [
    ...
    foreach (['teacher3', 'teacher5'] as $email) {
        $this->actingAs($owner)
            ->post("/subjects/{$subject->id}/reviewers", [
...
it('[10] lets a student upload their report PDF for their team attempt', function () {
    ...
    // Attaching stages a draft; turning in submits it.
    ...
    Storage::disk('private')->assertExists($paper->fresh()->file_path);
});
...
it('[13][14] propagates schedule, room, panel and document changes', function () {
    ...
    // Swap a panel member: add teacher8 as a judge on the defense session. Panel
    // members are now managed per defense session (Manage Judges), not via team
    // membership, so the assignment runs through the defense-attempt route.
...
/* ------------------------------------------------------------------ *
 *  Subject owner auto-joins every defense attempt                    *
 * ------------------------------------------------------------------ */

it('auto-assigns the subject owner as an active reviewer on a newly created attempt', function () {
    $owner = User::where('email', 'teacher1@example.com')->firstOrFail();
    $team = Team::where('name', 'Team Solo 1')->firstOrFail();
    $period = DefensePeriod::where('subject_id', $team->subject_id)->where('type', 'midterm')->firstOrFail();

    $newAttempt = $team->defenseAttempts()->create([
    ...
    $assignment = DefenseAttemptReviewer::where('defense_attempt_id', $newAttempt->id)
        ->where('reviewer_id', $owner->id)
        ->first();

    expect($assignment)->not->toBeNull();
    expect($assignment->status)->toBe('active');
    expect($assignment->committee_role)->toBe('fyp_instructor');
    expect($team->fresh()->members()->where('users.id', $owner->id)->exists())->toBeTrue();
});

it('does not duplicate the owner reviewer assignment if the attempt is reloaded', function () {
    $owner = User::where('email', 'teacher1@example.com')->firstOrFail();
    $team = Team::where('name', 'Team Solo 2')->firstOrFail();
    $attempt = $team->defenseAttempts()->first();

    $before = DefenseAttemptReviewer::where('defense_attempt_id', $attempt->id)
        ->where('reviewer_id', $owner->id)
        ->count();
    $attempt->ensureOwnerIsReviewer();
    $attempt->ensureOwnerIsReviewer();
    $after = DefenseAttemptReviewer::where('defense_attempt_id', $attempt->id)
        ->where('reviewer_id', $owner->id)
        ->count();
...
it('blocks non-admins from the new admin reports and audit pages', function () {
    $teacher = User::where('email', 'teacher1@example.com')->firstOrFail();
    $this->actingAs($teacher)->get('/admin/reports')->assertForbidden();
    $this->actingAs($teacher)->get('/admin/reports/scores.csv')->assertForbidden();
    $this->actingAs($teacher)->get('/admin/audit/reviews')->assertForbidden();
    $this->actingAs($teacher)->get('/admin/audit/rubrics')->assertForbidden();
});

it('serves a scores CSV to admins with the expected header columns', function () {
    $admin = User::where('email', 'admin@example.com')->firstOrFail();

    $response = $this->actingAs($admin)->get('/admin/reports/scores.csv');
    $response->assertOk();
    expect($response->headers->get('Content-Type'))->toContain('text/csv');

    $body = $response->streamedContent();
    // Strip UTF-8 BOM for parsing
    if (str_starts_with($body, "\xEF\xBB\xBF")) {
        $body = substr($body, 3);
    }
    $firstLine = strtok($body, "\n");
    expect($firstLine)->toContain('Subject');
    expect($firstLine)->toContain('Team');
    expect($firstLine)->toContain('Final Score');
    expect($firstLine)->toContain('Pass/Fail');
});

it('renders the audit pages without error for an admin', function () {
    $admin = User::where('email', 'admin@example.com')->firstOrFail();
    $this->actingAs($admin)->get('/admin/audit/reviews')->assertOk();
    $this->actingAs($admin)->get('/admin/audit/rubrics')->assertOk();
});
